...
## 验证目标（参考 Noita 的 PVE 体验）
本阶段 PVE 验证的成功标准：
- **可玩**：玩家能在 30 秒内稳定击杀一波怪（不会秒躺/也不会完全无压力），参考 Noita 的战斗节奏。
- **可差异**：同一模板不同顺序能表现出明显差异（击杀速度、玩家剩余血量、掉落触发稳定性），参考 Noita 中调整法术顺序带来的玩法变化。
- **可观测**：输出清晰的统计（剩余怪物数、掉落、玩家 HP、关键触发次数），参考 Noita 的战斗统计。
...
- **common**：常见模块碎片、低级催化剂（参考 Noita 的常见法术）
- **elite**：含稀有模块碎片、中级催化剂（参考 Noita 的稀有法术）
- **pve_bonus**：奖励表（用于“印记击杀掉落加成”验证）

### 经济动作（PVE Only，参考 Noita）
本阶段允许“标记→击杀→掉落加成”的验证，但必须满足：
- 掉落内容来自服务器/配置表（不是玩家直接定义，参考 Noita 的掉落表）
- 只依赖可校验条件（例如目标带印记 tag）
- **模块解锁**：掉落模块碎片用于解锁新档位或新模板（参考 Noita 的收集法术机制）
...
## 如何判断“顺序确实改变了玩法”（参考 Noita）
建议在同一模板下做两种顺序对比，观察（参考 Noita 中调整法术顺序的影响）：
- **清怪速度与稳定性差异**：是否更容易把怪控在射程外/更容易集火（参考 Noita 的走位+施法节奏）
- **同等预算下“收益实现方式”变化**：先控后打 vs 先打后控（参考 Noita 中“先充能再投射”vs“先投射再充能”的差异）
- **施法节奏差异**：前摇/后摇/充能时间的组合影响施法频率（参考 Noita 的充能机制）
